...
var PRODUCTION_CONFIG = {
    ...
    externalApiKeys: __spreadArray(__spreadArray([], (process.env.EXTERNAL_API_KEY_1 ? [{
        ...
        }] : []), true), (process.env.EXTERNAL_API_KEY_2 ? [{
...
function mergeConfig(base, override) {
    var _a;
    return {
        server: __assign(__assign({}, base.server), override.server),
        openapi: __assign(__assign({}, base.openapi), override.openapi),
        externalApiKeys: override.externalApiKeys !== undefined ? override.externalApiKeys : base.externalApiKeys,
        database: __assign(__assign({}, base.database), override.database),
        frontend: {
            openapi: __assign(__assign({}, base.frontend.openapi), (_a = override.frontend) === null || _a === void 0 ? void 0 : _a.openapi)
        }
    };
}
/**
...
function printConfigInfo(config) {
    console.log('🔧 项目配置信息:');
    console.log("\uD83D\uDCCD \u670D\u52A1\u7AEF\u53E3: ".concat(config.server.port));
    console.log("\uD83C\uDF10 \u73AF\u5883: ".concat(config.server.nodeEnv));
    console.log("\uD83D\uDD17 CORS\u6E90: ".concat(config.server.corsOrigin));
    console.log("\uD83D\uDCCA \u65E5\u5FD7\u7EA7\u522B: ".concat(config.server.logLevel));
    console.log("\uD83D\uDD11 OpenAPI AccessKey: ".concat(config.openapi.accessKey ? '已配置' : '未配置'));
    console.log("\uD83D\uDD10 OpenAPI AccessSecret: ".concat(config.openapi.accessSecret ? '已配置' : '未配置'));
    console.log("\uD83C\uDFE2 OpenAPI BizType: ".concat(config.openapi.bizType));
    console.log("\uD83C\uDF0D OpenAPI BaseURL: ".concat(config.openapi.baseUrl));
    console.log("\uD83D\uDD11 \u5916\u90E8API Keys: ".concat(config.externalApiKeys.length, " \u4E2A"));
    console.log("\uD83D\uDCBE \u6570\u636E\u5E93: ".concat(config.database.url));
    // 验证配置
