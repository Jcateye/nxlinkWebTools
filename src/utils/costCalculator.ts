...
): { cost: number; T_u: number; T_u_eff: number } => {
  const T_u = r_u * T;
  const T_u_eff = ceilToStep(T_u, config.asrBillingStep);
  const cost = (T_u_eff / 60) * config.asrPricePerMin;
  return { cost, T_u, T_u_eff };
};
 ...
 * char_eff = CEILING(char_vendor, b_tts_char)
...
): { cost: number; T_b: number; charSelf: number; charVendor: number } => {
  const T_b = r_b * T;
  const charSelf = T_b * config.ttsCharPerSec;
  const charVendor = config.ttsVendorCharRatio * charSelf;
  const charEff = ceilToStep(charVendor, config.ttsBillingStep);
  const cost = (charEff / 1000) * config.ttsPricePer1kChar;
  return { cost, T_b, charSelf, charVendor };
};
...
export const computeCost = (
  ...
  const { T, r_b, r_u, q, n_llm: customNLLM } = behavior;
  ...
  const asrResult = computeASRCost(T, r_u, config);
  const ttsResult = computeTTSCost(T, r_b, config);
